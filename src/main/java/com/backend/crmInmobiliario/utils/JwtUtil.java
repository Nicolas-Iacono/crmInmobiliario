...
public class JwtUtil {
    ...
    private static final long ACCESS_TTL_MS = 3 * 60 * 60 * 1000; ;   // 3 horas
    private static final long REFRESH_TTL_MS = 7L * 24 * 60 * 60 * 1000; // 7 días

    private Algorithm alg() { return Algorithm.HMAC256(this.privateKey); }
    ...
    public String createRefreshToken(Authentication auth) {
        return createRefreshToken(auth.getPrincipal().toString());
    }
    ...
    public Long extractUserId(DecodedJWT decodedJWT){ return decodedJWT.getClaim("userId").asLong(); }
    public Claim getSpecifClaim(DecodedJWT decodedJWT, String claimName){ return decodedJWT.getClaim(claimName); }
    public Map<String, Claim> returnAllClaims(DecodedJWT decodedJWT){ return decodedJWT.getClaims(); }
